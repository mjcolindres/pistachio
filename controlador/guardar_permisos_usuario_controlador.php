	         ...
	         $checkModificar=0;
	    			...
	    			if ($Comborol>0 )
	    			{
        						...
        						foreach ($array_id as $identificador) {
        								 $sqlexiste=("SELECT count(id_permiso) as permiso  from tbl_permisos where id_rol='$Comborol' and id_objeto='$identificador'");
...
else
{
	...
	/*echo '<script> alert("  '.$identificador.'  ")</script>';*/
								...
								$resultadonombre = mysqli_fetch_assoc($conection->query($sqlnombre));
									        					  bitacora::evento_bitacora( $_SESSION['idUser'],$Id_objeto,'INSERTO' , 'EL PERMISO '.$resultadonombre['objeto'].'');
					...
					{
					header('location: ../vistas/crear_permiso_vista.php?msj=2');
										echo '<script type="text/javascript">
                                    swal({
                                       title:"",
